...
| Relay node: product, steps 1–4 of §13 | ~60% | step 1 and the profile edit; step 2 — a person gives the verdict in the panel (no model); step 3 — like, take-back, name refusal; step 4 — consent with the ephemeral half; the contract: 92 operations built, 46 spec (measured on the evening of 2026-09-24, after `POST /vault/pin` and `POST /identities/close`; `check-openapi.sh`, by parsing the YAML — `grep` undercounts operations taken through anchors); support is outside the §13 steps: `POST/GET /support`, the team's side, `db/039` |
...
by `db/039` (2026-09-22). Migrations in `relay/node/db` — 39 files, the last `047` (measured on the evening of 2026-09-24; was "37, the last `045`" after the squash [retired]).
