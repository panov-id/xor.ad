...
- **A frozen session does not extend the year — decided by quorum on 2026-09-25.** A live session is one whose `frozen_at` is empty, and a frozen session's way back (a support request, the same device raised by paper code) is not use: the node's guard does not write its `last_seen_at`, the mark stops at the moment of the freeze, and the year runs from there. Otherwise a lost phone writing to support once a day would keep the identity forever — even with requests the route refused. Rejected: counting the year by live sessions only (an identity whose only session was frozen by the tenth wrong PIN would be closed on the next pass rather than after a year, and the paper code would burn for the person trying to come back) and leaving it as it is (`support.frozen.bump`).
