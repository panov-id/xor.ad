...
| 2. Feed and geo | [~] migrations `db/025`–`029`; `POST/GET/DELETE /feed`, `/feed/density`, delivery by circles; **no moderating model wired**; a person gives the verdict: `GET /admin/feed-queue` and `POST …/publish`, `…/refuse` are built on the node (2026-09-22, `routes/feed_queue.ts`, three tests), the panel has no queue page yet — `publish()` is called only by hand, only the sweeper runs unattended (`lib/feed_verdict.ts`); the name check at first publication depends on it too; the author's age is computable — accepted cost P1; not rolled out | [~] core: a phrase and the feed with its cursor |
