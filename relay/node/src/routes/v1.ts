...
async function authenticate(req: Request): Promise<SecretKey | Response> {
  // Before the key is even looked at, and deliberately so. The daily quota below
  // is per key, cached for ten seconds, counted on each node separately, and
  // switches off when the database is unreachable — which means the moment the
