...
# --root позволяет проверить чужой репозиторий тем же кодом. Появился 28.08.2026,
# когда выяснилось, что сорок экранных пар витрин не проверялись **вовсе**: обход
# идёт от корня гейта, а витрины подключены симлинками, которые скрипт намеренно
# отбрасывает, чтобы не считать пары дважды. Отбрасывал он их вместе с проверкой.
if [ "${1:-}" = "--root" ]; then
  ROOT_DIR="$(cd "$2" && pwd)"
  shift 2
fi
