...
| Route | What it does | Origin |
|---|---|---|
| `POST /support` | a request `{body, email, nonce}` (`email` optional; `body` at most 2000 characters, `support.body.length`); **201** `{public_no}` — 10 Crockford base32 characters at once; the fourth in a day — **429** with the storefront's support address in `message`, `Retry-After` and `until` (`support.requests.day`, `support.frozen.day`) | **built** 2026-09-22, was: spec (agreed 2026-09-17) (schema `support_requests`) |
