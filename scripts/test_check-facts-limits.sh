...
# Малое число без своей единицы. Ровно тот случай, который панель ревью нашла
# 04.09.2026: table.span.short «20 минут, короткий срок стола» проходил зелёным,
# потому что «20» встречается в chat_RU.md 30 раз — а срока такого у стола нет,
# у него один срок молчания 60 минут. Здесь «20 часов» тоже нигде не стоит, и
# до правила «у числа меньше 100 требуется единица рядом» это было бы зелёным.
probe "$(printf 'probe.unitless\t20\tчасов\tсрок, которого нет\tничем\txor.ad/docs/chat_RU.md')"
expect 1 'не найдено в' 'малое число без своей единицы рядом — красный'

# Обратная сторона того же правила: перечисление с одной единицей в конце —
# запись законная, и краснеть на ней нельзя. protocol_RU.md:107 пишет набор
# сроков как «10 / 30 / 60 минут», и первая версия правила его не принимала.
probe "$(printf 'probe.enumerated\t30\tминут\tсрок из перечисления\tничем\txor.ad/docs/protocol_RU.md')"
expect 0 'числа сходятся везде' 'перечисление с единицей в конце — зелёный'
