...
(102 операций: построено 39, из спеки 7, предложено 56), протокол —
...
- **Имена 56 предложенных маршрутов согласуются одним заходом** (протокол §8 п. 2): всё, что в yaml стоит с
  `x-status: proposed`, названо мной, поведение — из канона. После согласия статус меняется на `spec`.
