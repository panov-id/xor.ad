...
**An open question visible from here.** The world is one, while a notice arrives under a brand. So a complaint filed through one storefront about a phrase whose author arrived through another finds no copy under the face it was filed through. **The untrue answer to that was removed on 2026-08-31** (`relay/node/db/014_dsa_notice_out_of_scope.sql`): the case used to be filed as "there was nothing to copy", telling the notifier the phrase had expired while it was alive and one query away. It now has a reason of its own — `out_of_scope`, "we did not look under this face" — and the notice stays in the moderator's queue with status `received`, like every other. That did not settle the boundary; it stopped the system lying about it.

**The deadline "before the first line of snapshot code" was missed: the code exists** (`relay/node/src/lib/dsa_snapshot.ts`, 2026-09-01) and scopes the copy by the storefront the notice arrived through — not because that was decided, but because there was no time to decide. **A review panel on 2026-09-03 showed that one boundary cannot serve all three surfaces**, which changes the question itself. For the feed and for tables the world is one: `brand` there is attribution and takes no part in retrieval (see earlier in this same section), so a complaint must follow the target rather than the storefront. For a venue's offer it is the opposite: `docs/offers/SPEC_EN.md` declares `brand` the column "any search is bounded by", so the storefront *is* the visibility boundary there, and a reporter who never saw the offer is complaining about something that does not exist for them. The choice is therefore a property of each surface, not one rule for all of them, and it belongs next to the surface — in `SNAPSHOTTABLE`, not in a single scope applied to every copy.

Separate from the boundary, and by the panel's account costlier than it: **what happens when the owner cannot be determined at all** — for a chat, for a notice with no `target_id`, for an expired target, and for a surface the schema does not yet have. Today that is the entire flow. Answering "then it is the platform's notice" means a tenant stops seeing its own complaints; answering "then the storefront it came through" keeps the present behaviour as the fallback. To be settled together with the boundary, not after it.
