#!/usr/bin/env python3
# Point neighbro.place at Bunny prod for go-live, PRESERVING email (MX/SPF/DKIM).
# apex via ALIAS (address-type, coexists with MX); www + api via CNAME.
...
BUNNY_LANDING = "neighbro-prod.b-cdn.net"
BUNNY_API = "neighbro-api-prod.b-cdn.net"
NS = None
...
root, err = call({"Command": "namecheap.domains.dns.getHosts", "SLD": "neighbro", "TLD": "place"})
...
def build(apex_mode):
    ...
    if apex_mode == "ALIAS":
        ...
        recs.append({"Type": "CNAME", "Name": "www", "Address": BUNNY_LANDING + "."})
        recs.append({"Type": "URL301", "Name": "@", "Address": "https://www.neighbro.place/"})
    recs.append({"Type": "CNAME", "Name": "api", "Address": BUNNY_API + "."})
    return recs

def apply(recs):
    p = {"Command": "namecheap.domains.dns.setHosts", "SLD": "neighbro", "TLD": "place", "EmailType": "MX"}
