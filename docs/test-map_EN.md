...
| Where | Cases | About |
|---|---|---|
| `relay/node/test` | 406 | storefronts, panel, tenancy, DSA, keys, limits, request signature, identity routes, the PIN counter, the first PIN, recovery by paper code, freezing a session, the identity sweeper |
| `testing/e2e` | 10 | the waitlist and storefront headers |
| `panel/src` | 29 | the panel, vitest: role permissions, providers, DSA reasons, the DSA queue filter, the feed queue's waiting column |
