...
- **Секреты** (env визарда): `HETZNER_TOKEN`/`VULTR_API_KEY`/`DIGITALOCEAN_TOKEN`,
  ...
  ключ с рабочими объектами, и скрипт говорит об этом каждую ночь). Зона
  заведена 11.09.2026: `xor-backups`, регион DE с репликой в SE — вторая копия
  переживает потерю региона, не выходя за пределы ЕС.
