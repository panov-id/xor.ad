...
**Пятьдесят седьмое — 20.09.2026, и не по вкусу.** `POST /recovery/confirm` заведён кворумом двух линз ревью при постройке шага 1: колонка `identities.signup_completed_at` была обещана каноном 10.09.2026, а заполнять её было нечем — все прочие вызовы регистрации случаются до показа бумажного кода, — и `identities.recovery_wrapped_key` не писала ни одна операция, то есть `POST /recovery/claim` возвращать было нечего. Имя следует правилу этого же документа: действие глаголом там, где ресурса нет. Владельцем оно не утверждалось: список из 56 перестал быть полным, и это записано здесь, а не подразумевается.
