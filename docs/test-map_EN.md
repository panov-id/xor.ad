...
| Where | Cases | About |
|---|---|---|
| `relay/node/test` | 570 | storefronts, panel, tenancy, DSA, keys, limits, request signature, identity routes, the PIN counter, the first PIN, recovery by paper code, freezing a session, the identity sweeper, the lock order |
| `testing/e2e` | 10 | the waitlist and storefront headers |
| `panel/src` | 32 | the panel, vitest: role permissions, providers, DSA reasons, the DSA queue filter, the feed queue's waiting column |
| `panel/tests/e2e` | 5 | the panel, Playwright against the local stand: waitlist, an invite, overflow at each width, a verdict in the feed queue (2026-09-22) |
| **Total** | **617** | **about the feed — 42 (sending, the verdict, delivery by intersecting circles, the grid and the bands, take-downs, terms, the density step, the Article 17 statement and the visible boundary sitting on the published centre and the per-identity read limit and the microsecond cursor, the counters row made on the spot `und` passing the language filter and the radius widening through the box index the capped density count, the old node during the 027 deploy the backfill on the same grid and the hourly density limit, 2026-09-21), about chat — 0; about the request signature — 12, about step 1 — 61: routes 34, the transfer 8, the sealing key 5, freezing 3, the sweeper 10 against a live Postgres, the shared brake 3 by the clock; plus 4 about connection timeouts, the sweepers for invitations and nonces, the counters a closure writes and the brake gauge read on the scrape (2026-09-21) and 5 about the queue's metrics (2026-09-20, overnight, after the review panel); plus 20 about the lock order share → sessions → identity (2026-09-25): identity routes 6, the transfer 6, the sweeper 8 — races that do not deadlock, a close with no live share, skipping held rows, and the skip and `storage_failed` counters published at zero; and 4 the same day: a frozen session does not extend the year 1, the sweeper's guards against mutations 3** |
...
| 1.6 | The first name is accepted at registration and visible to nobody before the first match (decision of 2026-09-22) | registration → `name_state = accepted`; a name change → `pending`, accepted with the phrase | **held** — "a new name waits for the queue, and comes out with the next phrase" (`feed_publish.test.ts`) |
...
| 23.5 | While stepped away every signed request except `DELETE /away`, `GET /identities/me`, `POST /identities/close`, `POST /support` and a repeat of `POST /away` with its own nonce → 409 `stepped_away` (§4.9) | `POST /away`, then `GET /feed` and `POST /chats/:id/ticket` | nothing to check |
...
| 23.8 | A repeat of any of the seven §2 routes (`/tables`, `/away`, `/support`, `/recovery/reissue`, `/identities/close`, `/vault/pin`, `/blocks`) with the same `nonce` within ten minutes answers the first answer and does not act twice (§2) | two identical requests | **held for three routes** — `/away`, `/vault/pin`, `/blocks`, a repeat from a time away included: "a repeated block is answered from a time away, and a new one is refused" (`feed_publish.test.ts`), "while away a repeat is answered, and a new change is refused" (`identity_routes.test.ts`), 2026-09-24; the other four are not built or not checked |
...
| 23.11 | What is liked leaves the delivery: after `POST /feed/:id/like` the phrase is absent from this person's `GET /feed` but present in `GET /likes` with `state: liked`; after `DELETE` it is back (2026-09-17) | a like, two deliveries, a take-back | **held** — "a liked phrase leaves my feed, not anybody else's, and waits in my likes", "a like taken back returns the phrase to the feed, and a block takes it out of the list" (`feed_publish.test.ts`) |
...
| 23.14 | The room of someone away is handed nothing; when the time away ends by itself the minute job `wake_returned` wakes it once, and a return by `DELETE /away` wakes it itself and is not woken again by the job (chat spec §8.2, `db/047`, 2026-09-24) | a line to someone away, the span moved into the past, two passes of the job; and the same with `DELETE /away` | **held** — "a room of someone away is handed nothing, and gets what waited once they are back", "a time away that runs out wakes the held rooms once, and a return by hand is not woken twice" (`feed_publish.test.ts`, listens on `chat_message`) |
| 23.15 | The `next` cursor of the feed and the likes is sealed: it shows neither the time nor the `id`; a cursor of the old shape, edited, or from the other list → 400 `invalid_body` (chat spec §8.11, protocol §6, 2026-09-24) | an issued cursor, one character of it changed, a `<microseconds>_<id>` pair, a likes cursor on the feed and back | **held** — "a page cursor says nothing, and only one issued for that list opens" (`feed_publish.test.ts`) |
| 23.16 | A body over 1048576 bytes → 413 `invalid_body` before any route and the signature check; counted on the stream, a lying or absent content-length does not help (protocol §6, 2026-09-24) | 2 MiB with a length, 2 MiB chunked, a small length declared over a 3 MiB stream, exactly 1 MiB passes whole | **held** — `body_limit.test.ts` (five cases); live on the stand's node 2026-09-24: 413 with a length and chunked |
