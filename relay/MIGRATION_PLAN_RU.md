...
> ✅ **ЗАВЕРШЕНО (2026-07-22).** Все 4 фазы выполнены: данные перенесены, sosed+neighbro
> лендинги и панель на relay, оба Supabase-проекта удалены (бэкап в
> `supabase-backup-2026-07-22/`). relay `v0.2.0` во всех env.
>
> Исторический документ. 29.07.2026 удалены одноразовые скрипты фаз
> (`migrate_waitlist.py`, `seed_panel_users.py`): они читали через
> `api.supabase.com`, а проектов больше нет — запустить их невозможно. Ссылки на
> них ниже оставлены как след того, чем переезд был сделан.
