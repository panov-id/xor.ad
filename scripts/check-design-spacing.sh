#!/usr/bin/env bash
# Spacing on the mock-up sheets — captions under buttons and the inset of a button in its block.
#
...
#
# Runs scripts/check-design-spacing.mjs in the Playwright image; nothing on the host.
...
docker run --rm \
  -u "$(id -u):$(id -g)" \
  -e ONLY="${1:-}" \
  -v "$root/scripts/check-design-spacing.mjs":/tests/check.mjs:ro \
  -v "$design":/design:ro \
