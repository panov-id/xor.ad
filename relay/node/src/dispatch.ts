// The node's answer to one request: the body's ceiling, CORS, the route table
// and the pattern router, HEAD as GET, the request's line and its count.
// main.ts serves it; a suite calls it with a Request and no socket, so what
// it checks is the dispatcher itself, not a copy of its rules.
...
export async function dispatch(incoming: Request, remote?: string): Promise<Response> {
