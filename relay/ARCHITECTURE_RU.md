...
Актуально для **neighbro** (мигрирован полностью). **sosed** ещё на Supabase.
...
## Что больше НЕ используется (для neighbro)
...
| Bunny proxy-зоны `api.dev/uat.neighbro.panov.id`, `api.neighbro.place` (→ Supabase) | ❌ лендинг их не зовёт (остаются «мёртвыми» в Bunny) |
...
| `push_subscriptions` (Supabase) | ⚠️ инертно: роута в relay нет, push выключен (`vapidPublicKey:""`); `supabaseUrl` в `config.js` остался опционально только под этот выключенный push |

## Нюансы

- **Supabase жив в целом** — им пользуются панель (`xor.panov.id`) и лендинг **sosed**
  (ещё не мигрирован). Отключился от Supabase только backend лендинга neighbro.
...
- `landing/SPEC_*.md` всё ещё описывают старый Supabase-контракт.
- sosed: завести Resend-аккаунт и мигрировать лендинг.
