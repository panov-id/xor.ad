...
| swipe left in the viewer = "minus", the phrase is gone for good | there is one reaction, the like; hide, block and report live in the "…" menu, and swiping on a card was considered and rejected: it fights the scroll (screen 5, 2026-08-26); in the viewer revisited on 2026-09-15 — the row below |
...
| column rails, `Refresh` and `Auto` (§6.1) | from 900px the feed and conversations, a third column only with a conversation open, next to the list from 1030px; no rails and no auto-refresh; "Say" and "Me" in the feed header (screen 3, 2026-09-15) |
