...
async function pair() {
  ...
  const aHalf = await aEph.publish(aLong.privateKey);
  const bHalf = await bEph.publish(bLong.privateKey);
  const a = await aEph.open(bHalf, bLong.publicSpki, CHAT, LOW, HIGH);
  const b = await bEph.open(aHalf, aLong.publicSpki, CHAT, HIGH, LOW);
...
Deno.test("a half that is not signed by the peer's long key is refused", async () => {
  ...
  const forged = await (await Ephemeral.generate()).publish(stranger.privateKey);
  await assertRejects(() => aEph.open(forged, bLong.publicSpki, CHAT, LOW, HIGH), Error, "not signed");
  // The right half with the wrong chat id gives different keys: nothing opens.
  const other = await aEph.open(bHalf, bLong.publicSpki, "another-chat", LOW, HIGH);
  const { b } = await pair();
