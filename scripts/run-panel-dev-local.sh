...
#
#   panel  -> http://localhost:<port> (default 62174)
#   relay  -> http://localhost:62080 (start it with relay/local/docker-compose.yml)
#
# Port 62173 is left to docker-compose.panel.yml, which points at the gateway
...
docker run --rm --name panel-dev-local \
  -v "$root/panel":/panel \
  -w /panel \
  -e VITE_RELAY_API_URL=http://localhost:62080 \
  -p "$port:$port" \
  node:20-alpine \
