...
|---|---|---|
| S1: лайк вслепую + `GET /likes` читают фразу мимо геофильтра | подтверждено; **принято как риск** (владелец, 23.09.2026): точку зрителя сообщает клиент и узел её не проверяет, так что проверка расстояния в лайке ничего не закрыла бы — записано в `chat_RU.md` §8.3; та же брешь раньше была через `POST/GET /hidden` | опровергатель: `hidden.ts:24-32` без расстояния — **ПРОВЕРЕНО** опровергателем, не мной |
...
1. ~~**S1 — лайк без проверки расстояния.**~~ Закрыто решением владельца 23.09.2026: принято как
   риск. Предложенная сначала точка в лайке оказалась пустой мерой — точку зрителя узел не
   проверяет и в `GET /feed`; записано в `chat_RU.md` §8.3.
