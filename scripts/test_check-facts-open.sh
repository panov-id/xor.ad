...
# Считается относительно живого реестра: 18.09.2026 в нём появился первый настоящий
# пункт, держащий отрисовку, и литерал «1» стал бы ложью при каждом следующем.
base=$(FACTS_OPEN="$witness" bash "$gate" 2>&1 | grep -o 'держит отрисовку [0-9]*' | grep -o '[0-9]*$'); base=${base:-0}
probe "$(printf 'probe.stops.drawing\t2026-08-31\t-\tproduct\tпроба\tchat\tdocs/facts/schema.tsv\t\tотрисовку')"
expect 0 "держит отрисовку $((base + 1))" 'пункт, держащий отрисовку, считается в итоге'
