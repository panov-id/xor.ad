...
- **A window instead of a nonce** is a deliberate trade (§8.2): whoever intercepts
  ...
  **Caveat of 2026-09-16 (panel, SEC-3):** for one-shot actions the price of a replay exceeds the
  ...
  shared by the pool and surviving a restart; the nonce is bound to its route, the same nonce on another route answers 409 `invalid_body` (SEC-25); the same nonce with another body on the same route is a repeat too: it gets the first answer and changes nothing — the body is signed, so it is the same device asking, and a new action wants a new nonce (decided by quorum on 2026-09-25); the replay is looked up after the signature and version checks and before the route's rate limit and the stepped-away refusal — a repeat is answered even when the hour's limit is spent or the person is away (2026-09-24, review panel of the loop); on `POST /identities/close` a repeat after a close that went through gets 401 — the close froze the session itself, and the request never reaches the replay lookup, and only 2xx and state-409 answers are kept (SEC-2, SEC-4 of pass 5); the "shared memory" argument is withdrawn for these seven
...
| Route | What it does | Origin |
|---|---|---|
| `POST /feed` | publishes a phrase; answers **202** at once, `visible_at` stays empty until the queue's verdict | **built** (`routes/feed.ts`; checked against the code 2026-09-24), was: spec |
| `GET /feed` | delivery by intersecting circles (a phrase in its last 65 minutes carries `soon` instead of its end, 2026-09-23), with the language, mode and age filters; what the viewer liked is not in it — it is in `GET /likes` (2026-09-17) | **built** (`routes/feed.ts`; checked against the code 2026-09-24), was: spec (named in the test map) |
| `DELETE /feed/:id` | withdraws your own phrase; the slot is freed, the hourly ceiling is not | **built** (`routes/feed.ts`; checked against the code 2026-09-24), was: spec (agreed 2026-09-17) (§8.3) |
| `GET /feed/density` | the density band under the radius handle: `nobody here yet` … `hundreds`, on release | **built** (`routes/feed.ts`; checked against the code 2026-09-24), was: spec (agreed 2026-09-17) (§8.3, screen 3) |

Each card in `items` of the feed answer (`{items, next}`, §6) carries `{kind, id, text, mode, lat, lon, area_radius, like_count}`; `kind` is `phrase` or `table`, and a table carries `{game, playing, watching}` instead of `text`. **Tables travel outside the cursor:** the random quarter (§6.1) is given only on the first page, when `after` is empty — a table has no `visible_at`, and the cursor cannot position it (2026-09-16, DATA-23). A phrase carries `{id, text, mode, lang, lat, lon, area_radius, like_count}`
(**`created_at` was removed on 2026-09-24, the owner's decision:** a phrase's span is fixed, so its start to the second is its end to the second, and §8.11 of the chat spec never gives out another person's end) — **a circle, not a point**, and nothing about the author. `lat`/`lon` are **rounded to a grid node stepped by `area_radius`** (the formula is in `chat_EN.md` §8.3); the exact ones never leave and, since 2026-09-21, **take no part in computing the overlap either**: the node compares against the same rounded pair it hands out (`relay/node/db/027`). Before that the inequality was measured against the exact pair, and a viewer — who supplies their own centre and radius as query parameters — could bisect the boundary and recover the exact centre to within metres.
...
| `GET /likes` | everything this identity liked that is still alive (a phrase in its last 65 minutes carries `soon`, 2026-09-23): cards of the same shape as `GET /feed` — phrases and private authors' offers with `state` (`liked` \| `matched`) and tables — by the `?after` cursor and `{items, next}` (§6); screen 25 "My likes". Newest like first, 30 a page, each card also carries `liked_at`; `matched` means a live match came out of the phrase; phrases of either side of a block and expired ones drop out | **built** 2026-09-23 for phrases (`routes/likes.ts`); tables and the one-sided match of an offer are not built. Was: spec (proposed and agreed 2026-09-17) (§8.4, mechanics §11) |
...
| `GET /inbox` | offers and conversations in one response; a count only on offers; a conversation carries one's own span `my_span` (2026-09-23), never the other side's | **built** 2026-09-21, was: spec |
...
`1006` is never sent by the node: under RFC 6455 §7.4.1 a client sees it when the line dropped with no Close frame. depth answers it as it answers `1001`: it reconnects after a pause (`depth/core/reconnect.ts`, 2026-09-24).
...
| Route | What it does | Origin |
|---|---|---|
| `GET /statements` | your own Article 17 statements without the notifier's identity, fields as in `statement_of_reasons` of the DSA spec: `{id, restriction, until, facts, ground_kind, ground_text, automated_used, created_at, appeal}` (`until` absent — indefinite; `ground_kind` is `legal` or `contractual`); the first delivery sets `delivered_at` (LAW-1 of pass 3) — shown on the next entry with this identity, kept a year (`dsa/SPEC_EN.md` §7, the entity `statement_of_reasons` in §8, the retention in §9; corrected from §6 on 2026-09-20 — that section is the reply to the notifier under Art. 16(5), not the statement to the author); added 2026-09-16, LAW-1 | **built** (`routes/statements.ts`; checked against the code 2026-09-24), was: spec (proposed 2026-09-16, agreed 2026-09-17) (`dsa/SPEC_EN.md` §7) |
| `POST /report/decision` | the decision on a notice by the device's receipt code, the code in the body; not signed by an identity; "no such receipt" and "not decided yet" get the same answer — status 200, the body byte for byte, `Cache-Control: no-store`, one minimum response time for both branches; the per-address limit lives in memory and is not logged; the code is 32 hex characters (128 bits), `receipt_hash` on `POST /report` is the SHA-256 of that string; the answer is `{decided: false}` or `{decided: true, outcome: upheld or rejected, automated, decided_at}`, with no reason for the notifier — there is no column for it (the body decided by quorum on 2026-09-24) — **built** (`routes/report.ts`, `db/052`; checked against the code 2026-09-24), was: spec (agreed 2026-09-17) (final panel, SEC-9) | **spec** (agreed 2026-09-17) (`dsa/SPEC_EN.md` §6); the answer's body — decided by quorum on 2026-09-24 |
...
| Route | What it does | Origin |
|---|---|---|
| `GET /identities/me` | your own profile: `{name, name_pending, age, filter_age_min, filter_age_max, languages, stepped_away_until, phrases: [{id, expires_at}], table: {id, seat}}` — your own live things with their timers for screens 9 and 10 (CON-17) | **built** without `quota` and `table`; `phrases` since 2026-09-23, a phrase waiting for the queue without `expires_at`; was: spec (proposed 2026-09-16, agreed 2026-09-17) (screen 10) |
| `PATCH /identities/me` | edit `{name, age, filter_age_min, filter_age_max, languages, filter_modes}` (`filter_modes` — 2026-09-19) — any subset; a name goes to the moderation queue (**202**, the verdict as a `name_verdict` frame), refusal `name_frozen` with a live phrase or an open chat and `paused` during the pause after refusals; age across the 20/21 border only upwards, `age_step_down`; the filter — bounds anything inside the band (by the year, 2026-09-17), not wider than the band — `filter_out_of_band`; up to three languages (`identities.languages`); at most `profile.patch.day` edits a day per identity | **built** 2026-09-22 (`routes/profile.ts`; the `name_verdict` frame — since 2026-09-24 to the author's open rooms, §4.4; `filter_modes` — no, it answers 400), was: spec (agreed 2026-09-17) (§8.2, §8.3; `name.length`, `profile.patch.day`; `filter.age.step` and `filter.age.min_width` were retired 2026-09-17 and are not in the registry — panel 2026-09-22) |
| `GET /identities/appearance` | read the appearance `{theme, contrast, accent}` kept on the node; the device copy draws the first frame (screen 22; 2026-09-19); nothing chosen is every field `null` | **built** 2026-09-23 (`routes/appearance.ts`), was: spec |
...
`code` is a machine name from the closed list in `docs/api/openapi.yaml` (`ApiError`), `message` is text for a person in the request's language, `reason` appears only on moderation refusals and carries a text from `refusal-wordings_EN.md`. The 409 "accept again" refusal does not move into this shape: it has its own, named above; the client tells the two bodies under one 409 apart by the presence of `error.code` — the re-acceptance has none, and `POST /feed` and the like are described in `openapi.yaml` as `oneOf` (CON-6). The codes `stale_seq` and `nothing_to_undo` were added 2026-09-16 (DATA-24, OPS-12). **A rate limit is a 429 in the same shape with `code: rate_limited` and a `Retry-After` header in seconds**, as the built `POST /report` and `POST /waitlist` do (`report.ts`, `waitlist.ts`). **The protocol version is the header `x-protocol-version` carrying the integer major version** on every request; an unsupported one answers **400** `protocol_version_unsupported` with the update command in `message`; the header is not signed, and forging it only refuses that same request. **The body of any request is capped at 1048576 bytes (1 MiB, 2026-09-24) and 30 seconds:** not all there in time — **408** `invalid_body`; cut off mid-body — **400**; the refusal carries the CORS headers. The body is counted on the stream before any route or the signature check reads it, content-length or not; over it — **413** `invalid_body`. The node takes no files, and its largest honest body is the text of a notice's reason or a support answer (`relay/node/src/lib/body_limit.ts`). **Pagination is a cursor:** `GET /feed`, `GET /inbox` and `GET /likes` take `?after=<opaque string>` and answer `{items, next}`; an empty `next` is the end; the cursor encodes (`visible_at`, `id`) for the feed, (`created_at`, `id`) for the inbox (DATA-5) and (the time of the like, `id`) for the likes (2026-09-23). **For the feed and the likes the pair is sealed since 2026-09-24** (the owner's decision, chat spec §8.11): AES-256-GCM under a key shared by the pool (`relay/node/src/lib/cursor.ts`), the list's name bound in as additional data; the cursor reveals nothing, and one the pool did not issue for that list — edited, forged, of the old `<microseconds>_<id>` shape or from the other list — answers **400** `invalid_body`. Was: "reveals nothing beyond what was already given out" [retired] — untrue, the clear pair carried another person's phrase's `visible_at` to the microsecond, and with a fixed span that is its end. The inbox cursor stays in the clear: it holds the times of one's own matches and conversations. The intake routes from the storefronts (`/waitlist`, `/report`, `/pageview`) keep the flat string `{error: "…"}` — they are built and not signed by an identity.
