#!/usr/bin/env bash
# Все ворота проекта одной командой.
#
#   scripts/check-all.sh              ворота
#   scripts/check-all.sh --with-tests ворота и пробы самих ворот
#
# Зачем. Ворот стало столько, что «прогнать перед докладом» перестало быть
# действием и стало намерением: часть гоняется, часть забывается, и забытая —
# ровно та, которая нашла бы. Здесь их список один, и он же отвечает на вопрос,
